...
This code was stolen from Max Weidmann, who understands for loops better than I do and texted me an example, 
which I modified to be applicable here. Mwahahaha stealing from siblings >:]
*/
...
void loop() {
  for (a = 0; a < 6; a++) {
    memory [0] [a] = random(0,2); //fill the first row with random bools. This will be DHT data later.
  }
...
/*
This test is really just to see if I can move things around in an array the way I need. 
It's a new skill for me, and this isn't the most simple introduction to the idea.

RESULT: good enough!
I'm mildly worried about an edge case of a row change invalidating a column, 
especially the first (0th) column, but not enough to change the code about it.

This indicates the arduino is capable of holding, referencing, and overwriting a little matrix,
as well as indicating my capability for understanding how to do that!
*/